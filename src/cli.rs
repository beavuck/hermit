use crate::constants::{DEFAULT_MAX_ITEMS, DEFAULT_MIN_ITEMS, DEFAULT_PORT, DEFAULT_SPEC_GLOB};
...
mod tests {
    ...
    fn min_items_and_max_items_can_be_configured_via_cli() {
        let args =
            Args::try_parse_from(["hermit", "--min-items", "5", "--max-items", "10"]).unwrap();
    ...
    fn validate_rejects_min_greater_than_default_max() {
        ...
        let args = Args::try_parse_from(["hermit", "--min-items", &min]).unwrap();
    ...
    fn validate_rejects_max_less_than_default_min() {
        ...
        let args = Args::try_parse_from(["hermit", "--max-items", &max]).unwrap();
        assert!(args.validate().is_err());
    }
    ...
    fn validate_rejects_min_greater_than_max() {
        let args =
            Args::try_parse_from(["hermit", "--min-items", "10", "--max-items", "3"]).unwrap();
        assert!(args.validate().is_err());
    }
    ...
    fn validate_accepts_equal_min_and_max() {
        let args =
            Args::try_parse_from(["hermit", "--min-items", "5", "--max-items", "5"]).unwrap();
    ...
    fn validate_accepts_max_below_default_min_when_min_is_also_set_lower() {
        ...
        let args =
            Args::try_parse_from(["hermit", "--min-items", &val, "--max-items", &val]).unwrap();
        assert!(args.validate().is_ok());
    }
    ...
    fn validate_accepts_min_above_default_max_when_max_is_also_set_higher() {
        ...
        let args =
            Args::try_parse_from(["hermit", "--min-items", &min, "--max-items", &max]).unwrap();
        assert!(args.validate().is_ok());
    }
}
